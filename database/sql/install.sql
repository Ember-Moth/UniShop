...
-- ----------------------------
BEGIN;
INSERT INTO `system_configs` (`key`, `value`, `description`, `type`, `options`, `group`, `sort_order`, `is_public`, `created_at`, `updated_at`) VALUES
-- 货币配置
('currency_unit', 'CNY', '默认货币单位', 'select', '{"CNY":"人民币","USD":"美元","USDT":"USDT","AUD":"澳元","EUR":"欧元","JPY":"日元","GBP":"英镑"}', 'currency', 1, 1, NOW(), NOW()),
('currency_symbol', '¥', '默认货币符号', 'select', '{"¥":"¥ (人民币)","$":"$ (美元)","€":"€ (欧元)","£":"£ (英镑)","¥":"¥ (日元)","A$":"A$ (澳元)"}', 'currency', 2, 1, NOW(), NOW()),
('supported_currencies', '["CNY","USD","USDT","AUD"]', '支持的货币列表', 'textarea', NULL, 'currency', 3, 1, NOW(), NOW()),
('currency_symbols', '{"CNY":"¥","USD":"$","USDT":"$","AUD":"A$"}', '货币符号映射', 'textarea', NULL, 'currency', 4, 1, NOW(), NOW()),

-- 汇率API配置
('exchange_rate_api_key', '', '汇率API密钥', 'text', NULL, 'exchange', 1, 0, NOW(), NOW()),
('exchange_rate_api_url', 'https://api.exchangerate.host', '汇率API地址', 'text', NULL, 'exchange', 2, 0, NOW(), NOW()),
('exchange_rate_update_interval', '3600', '汇率更新间隔（秒）', 'number', NULL, 'exchange', 3, 0, NOW(), NOW()),
('exchange_rate_cache_time', '3600', '汇率缓存时间（秒）', 'number', NULL, 'exchange', 4, 0, NOW(), NOW()),

-- 其他系统配置
('system_name', '独居客', '系统名称', 'text', NULL, 'general', 1, 1, NOW(), NOW()),
('system_version', '1.0.0', '系统版本', 'text', NULL, 'general', 2, 1, NOW(), NOW()),
('maintenance_mode', '0', '维护模式', 'switch', NULL, 'general', 3, 1, NOW(), NOW()),
('maintenance_message', '系统维护中，请稍后再试', '维护模式消息', 'textarea', NULL, 'general', 4, 1, NOW(), NOW());
COMMIT;
