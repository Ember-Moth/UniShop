 ...
 * Date: 2021/5/30
 */
class AgentOrderProcessService
{
    ...
    /**
     * 余额服务层.
     * @var \App\Service\BalanceService
     */
    ...
    public function setAgentOrderSn($agentOrderSn='')
    {
        $this->isAgent=empty($agentOrderSn)?0:1;
        $this->agentOrderSn=$agentOrderSn;
    }
     ...
     * @link      http://utf8.hk/
     */
    protected function calculateTheWholesalePrice(): float
    {
     ...
     * @link      http://utf8.hk/
     */
    public function calculateTheTotalPrice(): float
    {
    ...
    public function createOrder(int $user_id=0): Order
    {
        try {
            ...
            if($userBalance < $this->calculateTheTotalPrice()){
                throw new RuleValidationException('余额不足');
            }
...
//            }
            ...
            $order->trade_no = $order->order_sn;
            $order->is_agent = $this->isAgent;
            $order->agent_order_sn = $this->agentOrderSn;
            // 保存订单
            $order->save();

            //扣余额
            $this->balanceService->decreaseBalance($order->user_id,$order->actual_price,BalanceLog::TYPE_AGENT_CONSUME,$order->id);
